...
func GetFx() (string, interface{}, error) {
	log.Println("AxaFleetEmit")
	log.Println(os.Getenv("env"))
	var path []byte
	var excel [][]interface{}
	var e error
	path = lib.ErrorByte(ioutil.ReadFile("./positive-apex-350507-33284d6fdd55.json"))
	...
	_, e = CreateExcel(excel, "./tmp/"+filepath)
...
func SftpUpload(filePath string) {
	...
	pk = lib.ErrorByte(ioutil.ReadFile("./twayserviceKey.ssh"))
