...
func initCoverageP() *models.RuleOut {
	var coverages = make(map[string]map[string]*models.CoverageOut)
	offerPrice := make(map[string]map[string]*models.Price)

	coverages[monthly] = map[string]*models.CoverageOut{
		"IPI": {
			...
			Offer: map[string]*models.CoverageValueOut{
				base: {
					...
					DeductibleType:             "",
					SumInsuredLimitOfIndemnity: decimal.NewFromFloat(0.0),
					PremiumNet:                 decimal.NewFromFloat(0.0),
					SelfInsurance:              "0",
				},
				your: {
					Deductible:                 "0",
	...
	coverages[yearly] = map[string]*models.CoverageOut{
		"IPI": {
			Slug:                       "Invalidità Permanente Infortunio",
			Deductible:                 "0",
			Tax:                        decimal.NewFromFloat(2.5),
